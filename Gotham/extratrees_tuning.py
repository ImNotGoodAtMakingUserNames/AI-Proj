"""
Extra Trees (Extremely Randomized Trees) Hyperparameter Tuning
...
def tune_extra_trees(X, y, feature_names, cv_splits=5, n_iter=80, random_state=42):
    """
    Perform RandomizedSearchCV for Extra Trees hyperparameter tuning
    """
    ...
    print(f"\n{'='*70}")
    print(f"EXTRA TREES HYPERPARAMETER TUNING")
    print(f"{'='*70}")
...
def save_tuning_results(model_name, tuning_results, importance_df, log_dir='./results_logs'):
    ...
    params_path = os.path.join(log_dir, f'extratrees_best_params_{timestamp}.json')
    ...
    if importance_df is not None:
        importance_path = os.path.join(log_dir, f'feature_importance_extratrees_{timestamp}.csv')
...
if __name__ == "__main__":
    ...
    taxi = df.sample(frac=0.2, random_state=42).copy()
    ...
    tuning_results = tune_extra_trees(
    ...
    save_tuning_results('tuned_extratrees', tuning_results, importance_df)
