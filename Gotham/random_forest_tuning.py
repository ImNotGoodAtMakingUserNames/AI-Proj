"""
Random Forest Two-Pass Tuning with Feature Selection
...
def engineer_features(df):
    ...
    # Polynomial distance features (squared and cubed)
    ...
    # Interaction: Distance × Hour (traffic patterns)
    ...
    # Interaction: Hour × Is_Weekend (rush hour effects)
    taxi['hour_x_is_weekend'] = taxi['start_hour'] * taxi['is_weekend']
    
    # Interaction: Distance × Passengers (passenger count affects stops)
    ...
    # Interaction: Passengers × Is_Weekend (group trips on weekends)
    taxi['passengers_x_weekend'] = taxi['NumberOfPassengers'] * taxi['is_weekend']
    
    # Interaction: Time of Day × Season (seasonal rush hour patterns)
    taxi['timeOfDay_x_season'] = taxi['timeOfDay'] * taxi['season']
    
    # Start point quadrant (simplified geospatial grouping)
    ...
    # Cross-quadrant trips (long distance indicator)
...
def tune_random_forest(X, y, feature_names, pass_num=1, cv_splits=5, n_iter=50, random_state=42):
    """
    Perform RandomizedSearchCV for Random Forest hyperparameter tuning
    ...
    Returns:
        Dictionary with best model, params, and CV results
    """
    ...
    print(f"Testing {n_iter} random parameter combinations with {cv_splits}-fold CV")
...
def save_tuning_results(pass_num, tuning_results, importance_df, log_dir='./results_logs'):
    ...
    params_path = os.path.join(log_dir, f'random_forest_best_params_pass{pass_num}_{timestamp}.json')
...
def plot_feature_importance(importance_df, top_n=25, save_path=None):
...
def plot_hyperparameter_search_results(search_results_df, save_path=None):
    """Plot tuning results - CV score vs iteration"""
    fig, ax = plt.subplots(figsize=(12, 6))
    
    search_results_df = search_results_df.sort_values('rank_test_score')
    iterations = range(len(search_results_df))
    
    ax.plot(iterations, search_results_df['mean_test_score'].values, 'o-', label='Mean CV R²', linewidth=2)
    ax.fill_between(iterations, 
                     search_results_df['mean_test_score'].values - search_results_df['std_test_score'].values,
                     search_results_df['mean_test_score'].values + search_results_df['std_test_score'].values,
                     alpha=0.2, label='±1 Std Dev')
    
    ax.axhline(y=0.90, color='r', linestyle='--', label='Target R² = 0.90', linewidth=2)
    ax.axhline(y=0.80, color='orange', linestyle='--', label='Current Best R² ≈ 0.80', linewidth=2)
    
    ax.set_xlabel('Ranked Iteration (by CV R²)')
...
def plot_feature_selection_progress(history_df, save_path=None):
    """Plot feature selection progress"""
    fig, ax = plt.subplots(figsize=(12, 6))
    
    ax.plot(history_df['iteration'], history_df['cv_r2'], 'o-', linewidth=2.5, markersize=8, label='CV R²')
    ax.axhline(y=0.90, color='r', linestyle='--', label='Target R² = 0.90', linewidth=2)
    
    # Mark best point
    best_idx = history_df['cv_r2'].idxmax()
    ax.scatter([history_df.loc[best_idx, 'iteration']], 
              [history_df.loc[best_idx, 'cv_r2']], 
              color='gold', s=300, marker='*', 
              label=f"Best R² = {history_df.loc[best_idx, 'cv_r2']:.4f}", 
              zorder=10, edgecolors='black', linewidth=2)
    
    ax2 = ax.twinx()
    ax2.plot(history_df['iteration'], history_df['num_features'], 's--', linewidth=2.5, 
            markersize=8, color='green', label='# Features')
    ...
    ax.set_title('Feature Selection Progress (Iterative Removal)', fontsize=12, fontweight='bold')
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("RANDOM FOREST TWO-PASS TUNING WITH FEATURE SELECTION")
    print("="*70)
    ...
    # Sample - use 30% for more thorough testing
    ...
    tuning_results_pass1 = tune_random_forest(
    ...
    tuning_results_pass2 = tune_random_forest(
    ...
    print(f"  Best Max Depth: {tuning_results_pass1['best_params'].get('max_depth')}")
    print(f"  Best N Estimators: {tuning_results_pass1['best_params'].get('n_estimators')}")
    ...
    print(f"  Best Max Depth: {tuning_results_pass2['best_params'].get('max_depth')}")
    print(f"  Best N Estimators: {tuning_results_pass2['best_params'].get('n_estimators')}")
